...
export default {
  ...
  methods: {
    ...
    // 删除商品
    deleteCartById: async function (cart) {
      try {
    ...
    // 处理商品数量函数
    handler: throttle(async function (type, skuNum, cart) {
      ...
      try {
        await this.$store.dispatch('addOrUpdateShopCart', { skuId: cart.skuId, skuNum: skuNum });
        this.getData();
      } catch (err) {
  ...
  computed: {
    ...
    isAllChecked() {
      return this.cartInfoList.every(e => { return e.isChecked == 1 })
    }
  }
}
</script>
